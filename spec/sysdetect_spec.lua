...
describe("sysdetect", function()
   ...
   it("has a testcase for every binary sample", function()
   ...
   describe("detect_file", function()
      for f in lfs.dir("binary-samples") do
         if files[f] ~= "ignore" then
            ...
            it("detects " .. expected_s .. " " .. expected_p .. " sample", function()
      ...
      it("fails when given no argument", function()
         local pok, s, p = pcall(sysdetect.detect_file)
         assert.falsy(pok)
      end)
   end)
   ...
   describe("detect", function()
      ...
      it("detects running system when given no argument", function()
         local s, p = sysdetect.detect()
         assert.same("string", type(s))
         assert.same("string", type(p))
      end)
   end)
